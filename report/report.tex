...
\textbf{Answer: }
Since the website is a https secured website, we use the dns filter, rather than the http filter on the wireshark protocols list. The source and destination can be found in \ref{fig:dns-website} and are as follows
\begin{itemize}
	\item \textbf{Source: }172.17.61.200
	\item \textbf{Destination: }172.24.2.76
\end{itemize}
\begin{figure}[h!]
	\centering
...
\textbf{Answer: } You can see inFigure \ref{fig:http-browser-info}that the user agent field is populated with the browser I used, which is Mozilla Firefox. My operating system and system type is also sent, with \texttt{Ubuntu, Linux\_x86\_64} being seen.
\begin{figure}[h!]
	\centering
...
\textbf{Answer: }
The destination port, as can be seen in Figure \ref{fig:tcp-stats-info} is 80, and it's source port is 45675.
